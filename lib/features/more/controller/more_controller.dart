...
class MoreController extends GetxController {
  ...
  void onInit() {
    ...
    _setupDioDebug(); // ✅ DEBUG INTERCEPTOR
  ...
  // ✅ Dio request/response/error debug logs
  void _setupDioDebug() {
    ...
    _dio.interceptors.add(
      InterceptorsWrapper(
        onRequest: (options, handler) {
          ...
          log('🟦 [DIO REQ] headers=${options.headers}');
          handler.next(options);
        },
        onResponse: (response, handler) {
          log('🟩 [DIO RES] ${response.statusCode} ${response.requestOptions.uri}');
          handler.next(response);
        },
        onError: (e, handler) {
          log('🟥 [DIO ERR] ${e.requestOptions.method} ${e.requestOptions.uri}');
          log('🟥 [DIO ERR] type=${e.type} status=${e.response?.statusCode}');
          log('🟥 [DIO ERR] resp=${e.response?.data}');
          handler.next(e);
        },
      ),
    );
  ...
  // ✅ URL normalize (double slashes, backslash, trimming)
  String _normalizeFileUrl(String raw) {
    ...
    final base = ApiConstants.imageBaseUrl.trim().replaceAll(RegExp(r'\/+$'), '');
    final path = cleaned.replaceAll(RegExp(r'^\/+'), '');
    return '$base/$path';
  }

  // ✅ Safe URI encode (special chars in path)
  ...
  Future<void> fetchSupportThreads({
  ...
  // ✅ Always return Options + strong logging + stable download settings
  Future<Options> _buildDownloadOptions() async {
    final prefs = await SharedPreferences.getInstance();
    final token = (prefs.getString(userTokenKey) ?? '').trim();

    log('🟨 [AUTH] tokenPresent=${token.isNotEmpty} tokenLen=${token.length}');
  ...
  Future<void> sharePrescription({
    ...
    try {
      ...
      final resolvedUrl = _normalizeFileUrl(file);
      final uri = _safeUri(resolvedUrl);
      if (uri == null) {
        showToastMessage(message: 'Your prescription is invalid');
        return;
      }
      ...
      final options = await _buildDownloadOptions();
      await _dio.download(uri.toString(), path, options: options);
  ...
  // ✅ Download with FULL logs (400 body, url normalize, uri encode)
Future<String?> _downloadToTempFile({
  required String url,
  required String prefix,
}) async {
  ...
  log('🟦 [DL] savePath=$path');

  try {
    final response = await _dio.download(url, path);

    log('🟩 [DL] statusCode=${response.statusCode}');
    log('🟩 [DL] requestUri=${response.requestOptions.uri}');

    final file = File(path);
    final exists = await file.exists();
    log('🟩 [DL] fileExists=$exists');

    if (!exists) {
      log('🟥 [DL] File not created');
      return null;
    }

    final size = await file.length();
    log('🟩 [DL] fileSizeBytes=$size');

    if (response.statusCode != 200) {
      log('🟥 [DL] Non-200 status, download may have failed');
      try {
        final content = await file.readAsString();
        log('🟥 [DL] responseBodyPreview=$content');
      } catch (_) {}
      return null;
    }
    ...
    log('🟥 [DL] DioException: ${e.message}', stackTrace: s);
    log('🟥 [DL] status=${e.response?.statusCode}');
    log('🟥 [DL] respData=${e.response?.data}');
    return null;
  } catch (e, s) {
    log('🟥 [DL] Unknown error: $e', stackTrace: s);
    return null;
  }
}
  ...
  Future<void> openPrescriptionPreview({
    ...
    log('🟦 [PREVIEW] fileUrlRaw=$fileUrl');
    log('🟦 [PREVIEW] fileUrlTrim=$raw');
    ...
    try {
      ...
      if (localPath == null || localPath.isEmpty) {
        showToastMessage(message: 'Failed to download prescription');
        return;
      }

      log('🟩 [PREVIEW] opening localPath=$localPath');

      final result = await OpenFile.open(localPath);
      log('🟨 [PREVIEW] OpenFile result=${result.type.name} msg=${result.message}');

      if ((result.type.name).toLowerCase() == 'error') {
        showToastMessage(message: 'Failed to open prescription');
      }
    } catch (e, s) {
      log('🟥 [PREVIEW] openPrescriptionPreview error: $e', stackTrace: s);
      showToastMessage(message: 'Failed to open prescription');
    }
  }
}
