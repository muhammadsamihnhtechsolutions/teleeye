...
class _LiveSupportScreenState extends State<LiveSupportScreen> {
  late final TextEditingController _messageController;
  late final MoreController _supportController;
  ...
  void initState() {
    super.initState();
    _messageController = TextEditingController();
    _supportController = Get.isRegistered<MoreController>()
        ? Get.find<MoreController>()
        : Get.put(MoreController());
    _appointmentController = Get.isRegistered<AppointmentController>()
        ? Get.find<AppointmentController>()
        : Get.put(AppointmentController());

    WidgetsBinding.instance.addPostFrameCallback((_) async {

      // make sure appointments are loaded
      await _appointmentController.getAppointments();
      print('Appointments loaded for Live Support $_selectedAppointmentId');
      _supportController.initSupport(_selectedAppointmentId ?? "").whenComplete(_startPolling);
    });
  }
  ...
  void _startPolling() {
    ...
    _pollTimer = Timer.periodic(const Duration(seconds: 5), (_) async {
      if (!mounted) return;
      if (_supportController.isLoadingSupportMessages.value ||
          _supportController.isSendingSupportMessage.value) {
        return;
      }
      final supportId = _supportController.activeSupportId.value.trim();
      if (supportId.isEmpty) return;
      if (supportId != _lastSupportId) {
        _lastSupportId = supportId;
      }
      await _supportController.fetchSupportMessages(supportId);
    });
  }

  /// When user selects an appointment
  void _onAppointmentSelected(String? id) async {
    if (id == null) return;
    setState(() => _selectedAppointmentId = id);
    _supportController.supportMessages.clear();
    _supportController.activeSupportId.value = '';
    _supportController.supportThreads.clear();
    await _supportController.initSupport(id);
    // await _supportController.fetchSupportMessages(id);
  }
      ...
      body: SafeArea(
        child: Column(
          children: [
            ...
            Obx(() {
              final upcoming =
                  _appointmentController.upcomingAppointments.value;
              final followup =
                  _appointmentController.followupAppointments.value;
              final past = _appointmentController.pastAppointments.value;

              final allAppointments = [
                ...(upcoming?.appointmentList?.appointmentData ?? []),
                ...(followup?.appointmentList?.appointmentData ?? []),
                ...(past?.appointmentList?.appointmentData ?? []),
              ];
                      ...
                      Expanded(
                        child: DropdownButtonHideUnderline(
                          child: DropdownButton<String>(
                            ...
                            onChanged: _onAppointmentSelected,
                          ),
                        ),
                      ),
                    ],
            ...
            Expanded(
              child: Obx(
                () {
                  final isLoading =
                      _supportController.isLoadingSupportMessages.value;
                  final messages = _supportController.supportMessages;
                  if (isLoading && messages.isEmpty) {
                    return const Center(child: CircularProgressIndicator());
                  }
                  if (messages.isEmpty) {
                    return Center(child: _EmptyState(l10n: l10n));
                  }
                  final reversed = messages.reversed.toList();
                  return ListView.builder(
                    padding: EdgeInsets.symmetric(
            ...
            Obx(
              () => _MessageComposer(
                controller: _messageController,
                onSend: _handleSendMessage,
                isSending: _supportController.isSendingSupportMessage.value,
              ),
            ),
          ],
        ),
  ...
  void _handleSendMessage() {
    ...
    if (_selectedAppointmentId == null) {
      showToast(message: "Please select an appointment first", context: context);
      return;
    }
    _supportController.sendSupportMessage(_messageController.text.trim(), _selectedAppointmentId!);
    _messageController.clear();
  }
}

/// === Rest Widgets (unchanged) ===
      ...
      child: Row(
        children: [
          ...
          Container(
            ...
            decoration: BoxDecoration(
              color:
                  isSending ? AppColors.colorEDEDED : AppColors.primaryColor,
              borderRadius: BorderRadius.circular(12),
            ),
            child: IconButton(
              icon: const Icon(Icons.send, size: 18, color: Colors.white),
